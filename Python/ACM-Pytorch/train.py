...
for idx in range(args.num_splits):
    ...
    for epoch in range(args.epochs):
        ...
        model.eval()
        output,features3 = model(features, adj_low, adj_high, adj_low_unnormalized,h=True)
        save_path = f'output/{dataset}'
        os.makedirs(save_path, exist_ok=True) 
        torch.save(features3, os.path.join(save_path, f'{dataset}.pt'))
        if args.dataset_name == "deezer-europe":
