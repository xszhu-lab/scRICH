...
# def load_data_lll(dataset_name):
...
def load_full_data(dataset_name):
    if dataset_name in {"cora", "citeseer", "pubmed"}:
        ...
        # 增加没有关系的节点特征到图G中
            for key, value in graph_node_features_dict.items():
                if int(key) not in G:
                    G.add_node(
                        int(key),
                        features=value,
                        label=graph_labels_dict[key],
                    )
...
def train_prep(logger, args):
    ...
    if args.dataset_name == "custom_lll":
        adj_low_unnormalized, features, labels = load_data_lll(args.dataset_name)
    else:
    ...
    if args.dataset_name == "deezer-europe":
        ...
        split_idx_lst = None, #load_fixed_splits(args.dataset_name, "")
