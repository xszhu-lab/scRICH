...
def rcpmg_sp(norm_adata, sp_adata, n_cores, p_value, local_region, approximation, n_permutations):
    #多线程
    if multiprocessing.get_start_method(allow_none=True) != 'fork':
        multiprocessing.set_start_method("spawn", force=True)

    background = sp_adata.X
    #判断是否是稀疏矩阵，如果是稀疏矩阵则转换成密集矩阵
    if sp.issparse(background):
        background = background.toarray()
    #
    gene_expressions = [background[:, i].flatten() for i in range(np.shape(background)[1])]
    knn_matrix = sp_adata.obsp["connectivities"].toarray()
    ...
    norm_adata.var.drop(columns="RCPMG_sp_p_value", inplace=True, errors='ignore')
    norm_adata.var.insert(0, "RCPMG_sp_p_value", p_values_output)
